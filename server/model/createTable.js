...
// 点赞表
query('CREATE TABLE thumbs (thumbsId INT NOT NULL auto_increment primary key, dailyId INT, authorId INT, createTime datetime)').then(() => {
  console.log('创建thumbs表成功');
});

// 评论表
query('CREATE TABLE comment (commentId INT NOT NULL auto_increment primary key, dailyId INT, authorId INT, parentId INT, createTime datetime, content VARCHAR(255))').then(() => {
